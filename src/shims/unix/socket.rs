...
trait EvalContextPrivExt<'tcx>: crate::MiriInterpCxExt<'tcx> {
    ...
    /// on output it points the non-truncated size of the written address in the
    /// `address_ptr` buffer.
    ///
    ...
    fn write_socket_address(
        ...
        if address_ptr == Pointer::null() {
            // When provided a null pointer, the client isn't interested in the address
            // and we don't need to write anything.
            return interp_ok(Ok(()));
        }

        if address_len_ptr == Pointer::null() {
            // When the `address_ptr` isn't a null pointer but the `address_len_ptr` is a
            // null pointer, this is a fault or undefined behavior depending on the target.
            if matches!(this.tcx.sess.target.os, Os::Linux | Os::Android) {
                // On Linux and Android an EFAULT error is returned when the `address_ptr`
                // is non-null but the `address_len_ptr` is null.
                return interp_ok(Err(LibcError("EFAULT")));
            } else {
                // The POSIX man page states that "[...] the length of this address shall be stored
                // in the object pointed to by address_len".
                // By the POSIX terminology reference, violating a shall precondition leads to
                // undefined behavior:
                // <https://pubs.opengroup.org/onlinepubs/9799919799/basedefs/V1_chap01.html>
                throw_ub_format!(
                    "{foreign_name}: writing a socket address but the length pointer is a null pointer"
                )
            }
        }
        ...
        let (address_buffer, address_layout) = match address {
            SocketAddr::V4(address) => {
                ...
                this.write_bytes_ptr(
                    ...
                    std::iter::repeat_n(0, address_buffer.layout.size.bytes() as usize),
                )?;

                let sin_family_field = this.project_field_named(&address_buffer, "sin_family")?;
                // For historical reasons the `sa_family_t` is 16 bits whilst the `AF_INET` constant
                // is stored in a C-int which is usually 32 bits. The `AF_INET` constant is
                // chosen such that this conversion is safe.
                #[expect(clippy::as_conversions)]
                let address_family = this.eval_libc("AF_INET").to_i32()? as i16;
                this.write_scalar(Scalar::from_i16(address_family), &sin_family_field)?;
                let sin_port_field = this.project_field_named(&address_buffer, "sin_port")?;
                this.write_scalar(Scalar::from_u16(port), &sin_port_field)?;
            ...
            SocketAddr::V6(address) => {
                ...
                // For historical reasons the `sa_family_t` is 16 bits whilst the `AF_INET6` constant
                // is stored in a C-int which is usually 32 bits. The `AF_INET6` constant is
                // chosen such that this conversion is safe.
                #[expect(clippy::as_conversions)]
                let address_family = this.eval_libc("AF_INET6").to_i32()? as i16;
                this.write_scalar(Scalar::from_i16(address_family), &sin6_family_field)?;
                let sin6_port_field = this.project_field_named(&address_buffer, "sin6_port")?;
                this.write_scalar(Scalar::from_u16(port), &sin6_port_field)?;
